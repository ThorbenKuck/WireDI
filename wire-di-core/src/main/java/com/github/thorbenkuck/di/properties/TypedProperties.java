...
import static com.github.thorbenkuck.di.properties.Keys.format;
...
public class TypedProperties implements AutoCloseable {
    ...
    public static <T> void addTypeConverter(Class<T> type, PropertyConverter<T> converter) {
    ...
    public void set(
        ...
        dataAccess.write(() -> properties.put(format(key), value));
    }
    ...
    public void setAll(@NotNull final Properties newProperties) {
        dataAccess.write(() -> {
            newProperties.stringPropertyNames().forEach(key -> {
                String formattedKey = format(key);
                this.properties.put(formattedKey, newProperties.getProperty(key));
            });
        });
    }
    ...
    public String get(@NotNull final String key) {
        return dataAccess.read(() -> {
            String property = this.properties.get(format(key));
