...
class OrderedTest {
	...
	private static Stream<Arguments> orderPermutations() {
		return generatePerm(new ArrayList<>(List.of(first, second, third, fourth)))
				.stream()
				.map(Arguments::of);
	}
	...
	public void testThatOrderingWorksOnInverseOrder(List<Ordered> input) {
		// Act
		List<Ordered> result = input.stream().sorted(OrderComparator.INSTANCE).collect(Collectors.toList());
		...
		assertThat(orderValues).containsExactly(Integer.MIN_VALUE, Integer.MIN_VALUE + 1, Integer.MIN_VALUE + 2, Integer.MAX_VALUE);
		assertThat(orderValues).containsExactly(Ordered.FIRST, Ordered.FIRST + 1, Ordered.FIRST + 2, Ordered.LAST);
		assertThat(orderValues).containsExactly(Ordered.FIRST, Ordered.after(Ordered.FIRST), Ordered.after(Ordered.after(Ordered.FIRST)), Ordered.LAST);
	}
	...
	public void testThatOrderingAnInverseListWorks() {
		...
		// Act
		List<Ordered> result = entries.stream().sorted(OrderComparator.INSTANCE).collect(Collectors.toList());
	...
	public void testThatOrderingAnAlreadyCorrectlyOrderListWorks() {
		...
		OrderedFirst first = new OrderedFirst();
		OrderedSecond second = new OrderedSecond();
		List<Ordered> entries = List.of(first, second);
	...
	private static class OrderedThird implements Ordered {
		@Override
		public int getOrder() {
			return Ordered.after(Ordered.after(FIRST));
		}
