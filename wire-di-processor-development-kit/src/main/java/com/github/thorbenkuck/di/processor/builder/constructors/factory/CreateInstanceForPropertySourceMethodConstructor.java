...
import com.github.thorbenkuck.di.runtime.properties.Keys;
...
public class CreateInstanceForPropertySourceMethodConstructor extends CreateInstanceMethodConstructor {
    ...
    protected void initialize(InjectionContext injectionContext, WireInformation wireInformation, CodeBlock.Builder methodBodyBuilder) {
        annotation = wireInformation.findAnnotationOnRoot(Properties.class)
        ...
        if (file.isEmpty()) {
            methodBodyBuilder.addStatement("$T properties = wireRepository.properties()", typedPropertiesClassName);
        } else {
