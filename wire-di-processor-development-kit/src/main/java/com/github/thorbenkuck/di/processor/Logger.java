...
public class Logger {
	...
	private static final List<BufferEntry> BUFFER_ENTRY = new ArrayList<>();

	static {
		if(ProcessorProperties.isEnabled(PropertyKeys.DEBUG_ENABLED)) {
			info("Debug enabled");
		}
		if(ProcessorProperties.isDisabled(PropertyKeys.WARN_REFLECTION_USAGE)) {
			warn("You have disabled warnings on reflection. It is understandable that the warnings are annoying, but there are different approaches than just disabling the warnings. Maybe take a look at those!");
		}
	}
	...
	public static void log(
		...
		if (messager == null) {
			synchronized (BUFFER_ENTRY) {
				BUFFER_ENTRY.add(new BufferEntry(targetElement, annotationMirror, annotationValue, logLevel, messageTemplate, argsForMessage));
			}
			return;
		}

		synchronized (BUFFER_ENTRY) {
			if(!BUFFER_ENTRY.isEmpty()) {
				BUFFER_ENTRY.forEach(entry -> writeLogMessages(
						entry.targetElement,
	...
	static void setLocalCurrentAnnotation(Class<? extends Annotation> currentAnnotation) {
		localCurrentAnnotation.set(currentAnnotation);
	}

	static void clearLocalCurrentAnnotation() {
	...
	private static class BufferEntry {
		...
		private BufferEntry(Element targetElement, AnnotationMirror annotationMirror, AnnotationValue annotationValue, Level logLevel, String messageTemplate, Object[] argsForMessage) {
