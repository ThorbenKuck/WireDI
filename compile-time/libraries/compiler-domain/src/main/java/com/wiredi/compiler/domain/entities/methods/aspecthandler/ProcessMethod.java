...
public class ProcessMethod implements MethodFactory {
	...
	private List<CodeBlock> getParameterList(List<ConstructorParameter> constructorParameterList) {
		...
		logger.info("Parameters on handler: " + methodDelegate.getParameters());
		methodDelegate.getParameters().forEach(parameter -> {
			logger.info(types.erasure(parameter.asType()) + " == " + executionContextType);
	...
	public void append(TypeSpec.Builder builder, AbstractClassEntity<?> entity) {
		final TypeMirror aroundType = Annotations.extractType(aspectAnnotation, Aspect::around);
		...
		builder.addSuperinterface(
				ParameterizedTypeName.get(
						ClassName.get(AspectHandler.class),
						TypeName.get(aroundType)
				)
		).addMethod(
				MethodSpec.methodBuilder("process")
						...
						.addParameter(
								ParameterSpec.builder(
												ParameterizedTypeName.get(
														ClassName.get(ExecutionContext.class),
														TypeName.get(aroundType)
												),
												"context"
						...
						.build()
		).addField(TypeName.get(handlerType.asType()), "delegate", Modifier.PRIVATE, Modifier.FINAL);
		...
		parameterList.forEach(parameter -> {
			builder.addField(FieldSpec.builder(parameter.typeName, parameter.fieldName, Modifier.FINAL).build());
			constructorBuilder.addParameter(parameter.typeName, parameter.fieldName);
			constructorBuilder.addStatement("this.$L = $L", parameter.fieldName, parameter.fieldName);
		});
